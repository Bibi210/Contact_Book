...
extern void data_base_init()
{
    ...
    if (sqlite3_exec(_db, "SELECT * FROM contact_list;", 0, 0, &errorsql) != SQLITE_OK)
    {
        ...
        sqlite3_exec(_db, "CREATE TABLE contact_list (id TEXT,nom TEXT,prenom TEXT,email TEXT,adresse TEXT,CP TEXT,type TEXT,num1 TEXT,num2 TEXT,num3 TEXT);", 0, 0, &errorsql);
    }
...
extern void data_base_add_contact(GList *un_type)
{
    ...
    sqlite3_prepare_v2(_db, "INSERT INTO contact_list (id,nom,prenom,email,adresse,CP,type,num1,num2,num3) VALUES(?,?,?,?,?,?,?,?,?,?);", -1, &commande, NULL);
